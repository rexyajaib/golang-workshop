...
import (
	"fmt"
	"golang-workshop/practice/tasks/helper"
)
...
func NewCoins() *Coins {
	return &Coins{
		Spot: make([]SpotCoin, 3), // Initialize slice
	}
}
...
func (c Coins) RemoveSpotCoin(index int) {
	helper.RemoveFromSlice(c.Spot, index)
}

func (c Coins) RemoveFutureCoin(index int) {
	helper.RemoveFromSlice(c.Future, index)
}
...
// DO NOT TOUCH
// Give summary explanation of what the code doing and the expectation
func main() {
	coins := NewCoins()
	...
	// Expect coins.Spot[0].Name to be "SOL" and coins.Future[0].Name to be "PERP-ETH"
	coins.LogCoins()
}
